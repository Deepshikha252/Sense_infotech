...
(function () {
  ...
  $('#formcty').submit(function (e) {
    e.preventDefault();
    var name    = $("#fnamecty").val();
    var email   = $("#emailcty").val();
    var message = $("#phonecty").val();
    var subject = $("#servicecty").val();
    var formData = new FormData(this);
    $.ajax({
      url: '../mailing/send_contact_data.php',
      type: 'POST',
      contentType: false,
      processData: false,
      data: formData,
      beforeSend: function () {
        $('#state_contact').attr('disabled',true);
        $('#state_contact').html('Sending...');
      },
      success: function (data) {
        console.log(data);
        if (data == 1) {
          $('#formcty').trigger('reset');
        }
      }
    }).done(function () {
        $('#state_contact').attr('disabled',false);
        $('#state_contact').html('Sent');
      })
  })
...
// js for float form date : 13-12-22
function opclo() {
